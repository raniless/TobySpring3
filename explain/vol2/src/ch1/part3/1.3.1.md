...
- ObjectFactory는 타입 파라미터와 getObject()라는 간단한 팩토리 메소를 갖고있다. 다음과 같이 사용
  ```java
  ObjectFactory<ServiceRequest> factory = ...;
  ServiceRequest serviceRequest = factory.getObject();
  ```
- ObjectFactoryCreatingFactoryBean는 ObjectFactory를 만들어주는 팩토리 빈
...
- 컨트롤러에서는 팩토리 인터페이스 타입으로 DI 받아서 사용하면 된다.
  ...
  public void serviceRequestFormSubmit(HttpServletRequest request) {
    ServiceRequest serviceRequest = serviceRequestFactory.getServiceFactory();
    serviceRequest.setCustomerNo(request.getParameter("custno"));  
  }
  ```
...
- Provider 인터페이스를 @Inject, @Autowired, @Resource 중 하나를 이용해 DI 되도록 지정해주기만 하면 스프링이 자동으로 Proivide를 구현한 오브젝트를 생성해서 주입해주기 때문.
